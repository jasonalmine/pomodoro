...
function Dialog({ req, close }: { req: ConfirmReq; close: () => void }) {
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-sm rounded-2xl bg-paper dark:bg-ink-900 border border-ink-200 dark:border-ink-800 p-5 shadow-xl"
      ...
      >
        ...
        {req.kind === 'prompt' && (
          <input
            ...
            className="mt-3 w-full rounded-xl border border-ink-200 bg-paper px-3 h-11 text-sm dark:bg-ink-900 dark:border-ink-700 dark:text-ink-100"
          />
        )}
