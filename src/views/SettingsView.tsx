...
function DataSection() {
  ...
  const onImport = async (file: File) => {
    ...
    try {
      ...
      setStatus({ kind: 'ok', text: `Restored ${r.projects} project${r.projects === 1 ? '' : 's'}, ${r.pomodoros} session${r.pomodoros === 1 ? '' : 's'}, ${r.tasks} task${r.tasks === 1 ? '' : 's'}${r.settingsRestored ? ', plus settings' : ''}.` })
    } catch (e) {
