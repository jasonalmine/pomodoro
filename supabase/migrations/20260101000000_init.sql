-- Baseline schema. Idempotent, so it can be applied to the existing
-- (manually-bootstrapped) database without error: tables/columns/indexes
-- use IF NOT EXISTS; policies are dropped then recreated.
--
-- After this baseline, every schema change is a NEW timestamped file in
-- this directory (npm run db:new "<name>"), applied with npm run db:push.
--
